package com.petmatz.infra.security.filter;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            Long userId = jwtVerification.validateAndGetUserId(token); // validate 메서드가 userId를 반환하도록 수정
            if (userId == null) {
                filterChain.doFilter(request, response);
                return;
            }
            ...
            // 사용자 ID로 사용자 정보 조회
            User userEntity = userQueryPort.findById(userId);
            if (userEntity == null) {
                filterChain.doFilter(request, response);
                return;
            }
            ...
            // 사용자 권한 설정
            LoginRole loginRole = userEntity.getAccount().getLoginRole();
